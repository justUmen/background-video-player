...
*/
  ...
  toggle_auto_change_image(){
    if(button_auto_change_image.classList.contains('status-added')){
      ...
      if(!button_random_image.classList.contains('status-added')
        && !button_random_image2.classList.contains('status-added')){
  ...
  default_image(file){
    ...
    //Minimap width :)
    backgroundImage.style.backgroundPosition="top 0px right "+document.querySelectorAll('.adjust-absolute-height')[0].offsetWidth+"px";
  ...
  async open_random_images(FILE_USER_IMAGES,WAIT=0) { //Wait x ms before changing wallpaper
    ...
    if (fs.existsSync(FILE_USER_IMAGES)) {
      ...
      if (fs.existsSync(atom.config.get('background-video-player.RI_path'))) {
        new Notification("Random wallpaper change", {
          body: "Use RI_path by default "+atom.config.get('background-video-player.RI_path')
        });
        button_random_image.classList.add('status-added');
        button_random_image2.classList.remove('status-added');
        this.open_random_images(atom.config.get('background-video-player.RI_path'));
        ...
        new Notification("Wallaper ", {
          body: "The folder "+atom.config.get('background-video-player.RI_path')+" does not exists..."
        });
      }
    }
  }
...
*/
  ...
  youtube(URL){
    ...
    //   atom-pane-container > atom-pane > div > video-player-background").shadowRoot.querySelector("video-player")
    //   .shadowRoot.querySelector("#background-video-player");
    // VIDEO.setAttribute('data-yt', URL);
    console.log("Youtube : " + URL);
    // new YouTubeToHtml5({
    //   withAudio: true
    // });
    var youtube_player = new YouTubeToHtml5({
    ...
    VIDEO=document.querySelector("body > atom-workspace > atom-workspace-axis > atom-workspace-axis > \
      atom-pane-container > atom-pane > div > video-player-background").shadowRoot.querySelector("video-player")
      .shadowRoot.querySelector("#background-video-player");
    VIDEO.setAttribute('data-yt2html5', URL);
    VIDEO.style.display="";
    // VIDEO.play();
    youtube_player.load();
    // VIDEO.play();
